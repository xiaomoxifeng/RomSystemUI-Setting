...
import java.text.NumberFormat;//grape_S6
...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode,
    ...
    private TextView mBatteryPercentage; //grape_S6
    ...
    protected PhoneStatusBarView makeStatusBarView() {
        ...
        mStatusBarWindow.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
            	Log.d("FLOW_ROM_TAG", "PhoneStatusBar onTouchEvent(MotionEvent ev) "+event.getAction());
        ...
        mBatteryPercentage = (TextView)mStatusBarView.findViewById(R.id.battery_level);
        mBatteryPercentage.setVisibility(View.VISIBLE);//grape_S6
        ...
        if (mQSPanel != null) {
            ...
            mQSPanel.setActivityStarter(this); //grape_S6
        ...
        //grape_S6 keyguard
        filter.addAction("TW_SET_LOCKSCREEN_WALLPAPER_SUCCESS");
        //end

        /*tom add S6*/
        filter.addAction("com.sec.android.intent.action.HOME_PAUSE");
        filter.addAction("com.sec.android.intent.action.HOME_RESUME");
...
//grape_S6 keyguard
        mNotificationPanel.setKeyguardViewManager(mStatusBarKeyguardViewManager);
        mNotificationPanel.setKeyguardMediatorCallback(mKeyguardViewMediatorCallback);
//end
    ...
    void makeExpandedVisible(boolean force) {
        ...
        if (false == force)
            mNotificationPanel.openQs();//grape_S6
    }
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            //grape_S6 keyguard
            else if ("TW_SET_LOCKSCREEN_WALLPAPER_SUCCESS".equals(action)) {
                    mNotificationPanel.onLockScreenWallpaperSet();
            }             
            //end
            else if ("com.sec.android.intent.action.HOME_PAUSE".equals(action)) //grape_launcher
            {
                if (DEBUG) {
                    Xlog.v(TAG, "freda com.sec.android.intent.action.HOME_PAUSE");
                }
                int mGestureEnabled = Settings.System.getInt(mContext.getContentResolver(), Settings.System.GESTURE_ACTIVE_SETTING, 1);
                int mGestureLauncherEnabled = Settings.System.getInt(mContext.getContentResolver(), Settings.System.GESTURE_AIR_LAUNCHER_ACCEPT_SETTING, 1); 	   
                int gestureStatus = GestureJNI.getPsGestureStatus();              
                Log.d("freda", "---HOME_PAUSE---mGestureEnabled=" + mGestureEnabled + " mGestureLauncherEnabled=" + mGestureLauncherEnabled + " gestureStatus=" + gestureStatus);
                if(gestureStatus == 4) {
                    GestureJNI.setGestureSensorEnable(0);
                }else if(gestureStatus == 2) {
                    broadcastGestureIntent(false);
                }

            } 
            else if ("com.sec.android.intent.action.HOME_RESUME".equals(action)) 
            {
                if (DEBUG) {
                    Xlog.v(TAG, "freda com.sec.android.intent.action.HOME_RESUME");
                }
                int mGestureEnabled1 = Settings.System.getInt(mContext.getContentResolver(), Settings.System.GESTURE_ACTIVE_SETTING, 1);
                int mGestureLauncherEnabled1 = Settings.System.getInt(mContext.getContentResolver(), Settings.System.GESTURE_AIR_LAUNCHER_ACCEPT_SETTING, 1); 	   
				int mGestureCrossTalk = Settings.System.getInt(mContext.getContentResolver(), Settings.System.GESTURE_LOW_DISTANCE_SETTING, 260);	
                int gestureStatus1 = GestureJNI.getPsGestureStatus();
                Log.d("freda", "---HOME_RESUME---mGestureEnabled1=" + mGestureEnabled1+ " mGestureLauncherEnabled1=" + mGestureLauncherEnabled1 + " gestureStatus1=" + gestureStatus1);
                if((mGestureEnabled1 == 1) && (mGestureLauncherEnabled1 == 1)) {
                    if(gestureStatus1 == 4) {
                        GestureJNI.setGestureSensorEnable(mGestureCrossTalk);
                    }else if(gestureStatus1 == 2) {
                        broadcastGestureIntent(true);
                    }
                }
            }            
        }
    ...
    private void updateClockSize() {
        ...
        if (clock != null) {
            FontSizeUtils.updateFontSize(clock, R.dimen.tw_status_bar_clock_size);
        }
    }
...
//end
    ...
    public void onVisibilityChanged(boolean visible) {
        ...
        if (visible) {
        	//intent.setAction(Constants.onShow);
        	//Log.d(Constants.FLOW_TAG, "onVisibilityChanged onShow");
            mSystemUiVisibility |= View.RECENT_APPS_VISIBLE;
        } else {
        	//Log.d(Constants.FLOW_TAG, "onVisibilityChanged onHide");
            mSystemUiVisibility &= ~View.RECENT_APPS_VISIBLE;
        }
   ...
   /// M: Support "Operator plugin - Customize Carrier Label for PLMN". @}
//tom_S6
    private void broadcastGestureIntent(boolean enable) {
        Intent broadcast;
        if(enable) {
            broadcast = new Intent("mac.intent.action.two_direction_gesture_enable");
        }else {
            broadcast = new Intent("mac.intent.action.two_direction_gesture_disable");
        }
