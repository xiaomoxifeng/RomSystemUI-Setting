...
//grape_S6 keyguard
import com.android.keyguard.sec.KeyguardEffectViewMain; 
...
public class KeyguardAffordanceHelper {
    ...
    public static final float SWIPE_RESTING_ALPHA_AMOUNT = 1.0f;
    ...
    private KeyguardEffectViewMain mKeyguardEffectViewMain;//grape_S6 keyguard
    private boolean swipHandled = false;
    private int swipHandledIndex = 0;
    private KeyguardAffordanceView mTargetView;
    private int mTrackingPointer;
    private static final String TAG = "KeyguardAffordanceHelper";
    ...
    private void initDimens() {
        ...
        mTouchSlop = 15;//configuration.getScaledPagingTouchSlop();
    ...
    private void initIcons() {
        ...
        mKeyguardEffectViewMain = mCallback.getUnlockEffectview(); //grape_S6 keyguard
    }


    private boolean isLeftSwipe(int i, int j)
    {
        Rect rect = new Rect();
        mLeftIcon.getGlobalVisibleRect(rect);
        boolean flag;
        if(mLeftIcon.getVisibility() == 0 && rect.contains(i, j))
            flag = true;
        else
            flag = false;
        return flag;
    ...
    private boolean isRightSwipe(int i, int j)
    {
        Rect rect = new Rect();
        mRightIcon.getGlobalVisibleRect(rect);
        boolean flag;
        if(mRightIcon.getVisibility() == 0 && rect.contains(i, j))
            flag = true;
        else
            flag = false;
        return flag;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        */
        int i = event.findPointerIndex(mTrackingPointer);
        if(i < 0)
        {
            i = 0;
        ...
        if(event.getActionMasked() == 0)
        {
            ...
            int k = (int)event.getRawY();
            if(isRightSwipe(j, k))
            {
                swipHandled = true;
                swipHandledIndex = -1;
                mTargetView = mRightIcon;
                //Log.d(TAG, "------------mRightIcon-------------");
            } 
            else
            if(isLeftSwipe(j, k))
            {
                swipHandled = true;
                swipHandledIndex = 1;
                mTargetView = mLeftIcon;
                //Log.d(TAG, "------------mLeftIcon-------------");
            }
        }
        if(!swipHandled && mKeyguardEffectViewMain != null)
        {
        	mKeyguardEffectViewMain.handleTouchEvent(event);
            return false;
        }
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                final float w = Math.abs(x - mInitialTouchX);
                final float h = Math.abs(y - mInitialTouchY);//grape_S6 keyguard
                trackMovement(event);
                /*if (((leftSwipePossible() && w > mTouchSlop)
                        || (rightSwipePossible() && w < -mTouchSlop))
                        && Math.abs(w) > Math.abs(y - mInitialTouchY)
                        && !mSwipingInProgress && swipHandled) */
                //Log.d(TAG, "ACTION_MOVE:mTouchSlop="+mTouchSlop+",mSwipingInProgress="+mSwipingInProgress+", w = "+w+", h = "+h);
                if(!mSwipingInProgress && (w > (float)mTouchSlop || h > (float)mTouchSlop) && swipHandled)        
                    { //grape_S6 keyguard
                    cancelAnimation();
                    mInitialTouchY = y;
                    mInitialTouchX = x;
                    ...
                    //Log.d(TAG, "------------setSwipingInProgress-------------mTouchSlop="+mTouchSlop);
                }
                float f4; //= Math.max(Math.abs((x + mTranslationOnDown) - mInitialTouchX), Math.abs((y + mTranslationOnDown) - mInitialTouchY));  //grape_S6 keyguard
                if (Math.abs((x + mTranslationOnDown) - mInitialTouchX) > Math.abs((y + mTranslationOnDown) - mInitialTouchY))
                    f4 = (x + mTranslationOnDown) - mInitialTouchX;
                else
                    f4 = (y + mTranslationOnDown) - mInitialTouchY;
                //Log.d(TAG, "ACTION_MOVE:y="+y+",mTranslationOnDown="+mTranslationOnDown+", mInitialTouchY = "+mInitialTouchY+", f4 = "+f4);
                if (mSwipingInProgress) {
                    setTranslationEx(f4, false, false);
                }
                break;
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                swipHandledIndex = 0;
                mTrackingPointer = -1;
                swipHandled = false;//grape_S6 keyguard
                break;
        }
        return true;
    }
    ...
    private void flingWithCurrentVelocity(boolean forceSnapBack) {
        ...
        boolean velIsInWrongDirection = false;//= vel * mTranslation < 0;
        //Log.d(TAG, "isBelowFalsingThreshold: mTranslation="+mTranslation+", vel="+vel+", velIsInWrongDirection="+velIsInWrongDirection + ",mMinFlingVelocity="+mMinFlingVelocity);
    ...
    private boolean isBelowFalsingThreshold() {
        //Log.d(TAG, "isBelowFalsingThreshold: mTranslation="+mTranslation+", mTranslationOnDown="+mTranslationOnDown+", getMinTranslationAmount="+getMinTranslationAmount());
    ...
    private void fling(float vel, final boolean snapBack) {
        ...
        if (!snapBack) {
            ...
            mCallback.onAnimationToSideStarted( (1 != swipHandledIndex)/*mTranslation < 0*/, mTranslation, vel);
        } else {
            reset(true);
        }
        animator.start();
    ...
    private void startFinishingCircleAnimation(float velocity, Runnable mAnimationEndRunnable) {
        ...
        if (1 == swipHandledIndex) targetView = mLeftIcon;
        else if(-1 == swipHandledIndex) targetView = mRightIcon;
        targetView.finishAnimation(velocity, mAnimationEndRunnable);
    }

//grape_S6 keyguard
	private void setTranslationEx(float translation, boolean isReset, boolean animateReset) {
	    translation = rightSwipePossible() ? translation : Math.max(0, translation);
	    translation = leftSwipePossible() ? translation : Math.min(0, translation);
	    //Log.d(TAG, "setTranslation:rightSwipePossible()="+rightSwipePossible()+ ",leftSwipePossible()="+leftSwipePossible()+ ",translation="+translation);
	    float absTranslation = Math.abs(translation);
	    if (absTranslation > Math.abs(mTranslationOnDown) + getMinTranslationAmount() ||
	            mMotionPerformedByUser) {
	        //Log.d(TAG, "getMinTranslationAmount()="+getMinTranslationAmount());
	        mMotionPerformedByUser = true;
	    }
	    if (translation != mTranslation || isReset) {
	        KeyguardAffordanceView targetView = mTargetView;
	        KeyguardAffordanceView otherView = mTargetView == mLeftIcon ? mRightIcon : mLeftIcon;
            ...
            Log.d(TAG, "setTranslation:radius="+radius+ ",absTranslation="+absTranslation);
	   
	        boolean slowAnimation = isReset && isBelowFalsingThreshold();
	        if (!isReset) {
	            updateIcon(targetView, absTranslation, alpha, false, false);
	        } else {
...
//end
    ...
    public void reset(boolean animate) {
        ...
        setSwipingInProgress(false);
        //if ((swipHandled) && (mTargetView != null))
          //mTargetView.showArrow(true);
    ...
    public interface Callback {
        ...
        KeyguardEffectViewMain getUnlockEffectview();
    }
}
