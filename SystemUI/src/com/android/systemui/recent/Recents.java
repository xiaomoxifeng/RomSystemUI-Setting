...
public class Recents extends SystemUI implements RecentsComponent {
    ...
    public void start() {
        ...
        registerRecentsFilter();
    }
    private void registerRecentsFilter() {
        IntentFilter filter = new IntentFilter();
        filter.addAction(Constants.onHideSystem);
        filter.addAction(Constants.onShowSystem);
        FlowShowLeftReceiver e =  new FlowShowLeftReceiver();
        mContext.registerReceiver(e,filter);
    }
...
/**
 * Flow showLift BroadcastReceiver
 */
class FlowShowLeftReceiver extends BroadcastReceiver {
    @Override
    public void onReceive(Context context, Intent intent) {
        if (intent.getAction().equals(Constants.onHideSystem)) {
           // hideLeftList(false);
        	Constants.isShowLeft=false;
        } else if (intent.getAction().equals(Constants.onShowSystem)) {
            //showLeftList();
        	Constants.isShowLeft=true;
        }
    }
}
