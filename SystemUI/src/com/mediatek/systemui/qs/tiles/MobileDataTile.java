...
public class MobileDataTile extends QSTile<QSTile.SignalState> {
    ...
    protected void handleClick() {
        if (mDataController.isMobileDataSupported() && mState.enabled) {
            ...
            if (!mState.connected) {
                ...
                if (subId < 0) {
                    Toast.makeText(mContext, R.string.application_guide_title, 2000).show();
                    return;
    ...
    protected void handleUpdateState(QSTile.SignalState state, Object arg) {
        state.visible = mController.hasMobileDataFeature(); //grape_S6
        if (DEBUG) {
            Log.d(TAG, "handleUpdateState mController.hasMobileDataFeature()=" + mController.hasMobileDataFeature());
        }        
        if (!state.visible) {
            return;
        }
        ...
        if (!state.enabled) {
            ...
            state.iconId = R.drawable.tw_quick_panel_icon_data_connection_off;

        } else if (dataConnected) {
            mDataConnectionState = DATA_CONNECT;
            mDataStateIconId = QS_MOBILE_ENABLE;
            state.iconId = R.drawable.tw_quick_panel_icon_data_connection_on;
