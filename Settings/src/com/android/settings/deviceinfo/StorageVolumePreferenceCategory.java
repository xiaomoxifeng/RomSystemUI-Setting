...
public class StorageVolumePreferenceCategory extends PreferenceCategory {
    ...
    public void updateApproximate(long totalSize, long availSize) {
      ...
      Log.d(TAG, "freda : total size is " + totalSize
                + ", avail size is " + availSize);

	  long totalSize_bak = totalSize;
	  Log.v(TAG, "freda updateApproximate: totalSize="+totalSize+",CategoryExts.mIsInternalSD="+mCategoryExts.mIsInternalSD+",isInternalVolume="+mCategoryExts.isInternalVolume() +",mVolume="+mVolume);	

        if  (true == FeatureOption.MACLAND_MEMORY_DUMMY_FAKE_FLAG)  {
	  
	  if (mCategoryExts.mIsInternalSD &&  111 != Build.getDummySdMemorySize() )//grape:add  2200936448L
	  {
	  	totalSize = (long)(((float)Build.getDummySdMemorySize()/*DUMMY_SDMEMORY_SIZE*//100f)/*8.82f*/*1024f*1024f*1024f);//17179869184L;
		availSize = availSize +  (totalSize/2 - totalSize_bak);//14023068222L;
             Log.v(TAG, "freda updateApproximate: ---1--- totalSize="+totalSize+",availSize="+availSize+",mIsInternalSD="+mCategoryExts.isInternalVolume() +",(totalSize - totalSize_bak)="+(totalSize - totalSize_bak));	
		if (availSize > totalSize)
			availSize = totalSize;
	  }
        else if ((mVolume == null) && (mCategoryExts.isInternalVolume() == true) && 111 != Build.getDummyDataMemorySize())
	  {
	  	totalSize = (long)(((float)Build.getDummyDataMemorySize()/*DUMMY_DATAMEMORY_SIZE*//100f)/*8.92f*/*1024f*1024f*1024f);//17179869184L;
		availSize = availSize + (totalSize/2 - totalSize_bak);//14023068222L;
		Log.v(TAG, "freda updateApproximate: ---2--- totalSize="+totalSize+",availSize="+availSize+",mIsInternalSD="+mCategoryExts.isInternalVolume() +",(totalSize - totalSize_bak)="+(totalSize - totalSize_bak));	
		if (availSize > totalSize)
			availSize = totalSize;
    ...
    public void updateDetails(MeasurementDetails details) {
	  ...
	  long totalSize_bak = details.totalSize;
		Log.v(TAG, "freda updateDetails: totalSize="+details.totalSize+",CategoryExts.mIsInternalSD="+mCategoryExts.mIsInternalSD+",isInternalVolume="+mCategoryExts.isInternalVolume() +",mVolume="+mVolume);   

        if (true == FeatureOption.MACLAND_MEMORY_DUMMY_FAKE_FLAG)  {
	  
	  if (mCategoryExts.mIsInternalSD && 111 != Build.getDummySdMemorySize())//grape:add  2200936448L  //grape_S6 fake
	  {
	  	details.totalSize = (long)(((float)Build.getDummySdMemorySize()/*DUMMY_SDMEMORY_SIZE*//100f)/*8.82f*/*1024f*1024f*1024f);
		details.availSize = details.availSize +  (details.totalSize/2 - totalSize_bak);
             Log.v(TAG, "freda updateDetails: --1--details.totalSize="+details.totalSize+",details.availSize="+details.availSize+",mIsInternalSD="+mCategoryExts.isInternalVolume() +",(totalSize - totalSize_bak)="+(details.totalSize - totalSize_bak));	
		if (details.availSize > details.totalSize)
			details.availSize = details.totalSize;
	  }
        else if ((mVolume == null) && (mCategoryExts.isInternalVolume() == true) && 111 != Build.getDummyDataMemorySize())
	  {
	  	details.totalSize = (long)(((float)Build.getDummyDataMemorySize()/*DUMMY_DATAMEMORY_SIZE*//100f)/*8.92f*/*1024f*1024f*1024f);
		details.availSize = details.availSize + (details.totalSize/2 - totalSize_bak);
		Log.v(TAG, "freda updateDetails: --2--details.totalSize="+details.totalSize+",details.availSize="+details.availSize+",mIsInternalSD="+mCategoryExts.isInternalVolume() +",(totalSize - totalSize_bak)="+(details.totalSize - totalSize_bak));	
		if (details.availSize > details.totalSize)
			details.availSize = details.totalSize;
