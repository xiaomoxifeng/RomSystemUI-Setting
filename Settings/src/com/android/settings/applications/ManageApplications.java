...
//grape_memory_dummy
        private    long freeStorage_bak = 0; //MACLAND_MEMORY_DUMMY_FAKE_FLAG
        private    long appStorage_bak = 0;
        private    long totalStorage_bak = 0;
//end
...
//A203 fix#bug				
if (true == FeatureOption.MACLAND_MEMORY_DUMMY_FAKE_FLAG && 111 != Build.getDummySdMemorySize())  {//grape_memory_dummy


	       totalStorage_bak = mTotalStorage;
		 freeStorage_bak = mFreeStorage;
	    	mTotalStorage =  (long)(((float)Build.getDummySdMemorySize()/*DUMMY_SDMEMORY_SIZE*//100f)/*8.82f*/*1024f*1024f*1024f);//12348030976L;//grape:add
		    mFreeStorage = mFreeStorage + (mTotalStorage - totalStorage_bak);
}
		    Log.i(TAG, "freda updateStorageUsage SD before totalStorage=" + mTotalStorage + ": freeStorage=" + mFreeStorage+",mAppStorage"+mAppStorage);		
            } else {
                if (mContainerService != null) {
                    try {
                        final long[] stats = mContainerService.getFileSystemStats(
...
if (true == FeatureOption.MACLAND_MEMORY_DUMMY_FAKE_FLAG && 111 != Build.getDummyDataMemorySize()) {  ////grape_memory_dummy grape_S6 fake
	       totalStorage_bak = mTotalStorage;
		 freeStorage_bak = mFreeStorage;

             mTotalStorage = (long)(((float)Build.getDummyDataMemorySize()/*DUMMY_DATAMEMORY_SIZE*//100f)/*8.92f*/*1024f*1024f*1024f);
		mFreeStorage = mFreeStorage + (mTotalStorage/2 - totalStorage_bak);	 
}			
		   Log.i(TAG, "freda updateStorageUsage DATA before totalStorage=" + mTotalStorage + ": freeStorage=" + mFreeStorage+",mAppStorage"+mAppStorage);				
            }
        ...
        void applyCurrentStorage() {
	        ...
	        if ((true == FeatureOption.MACLAND_MEMORY_DUMMY_FAKE_FLAG)  && 111 != Build.getDummySdMemorySize()) {//grape_memory_dummy grape_S6 fake
		   usedStorage = totalStorage_bak - freeStorage_bak;
		   Log.i(TAG, "updateStorageUsage  usedStorage=" + usedStorage + ": totalStorage_bak=" + totalStorage_bak+ ": freeStorage_bak=" + freeStorage_bak);
                }
