...
//      mSubscriptionManager.addOnSubscriptionsChangedListener(mSubscriptionListener);
    ...
    private void detectChangeAndNotify() {
        ...
        } else if (numSIMsDetected == 2) {
            if (!isDefaultDataSubInserted()) {
                Log.d(TAG, "two sim card---no slot is select---");
                Intent intent2 = new Intent(mContext, SimDialogActivity.class);
                intent2.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                intent2.putExtra(SimDialogActivity.DIALOG_TYPE_KEY, SimDialogActivity.PREFERRED_DATA_PICK);
                intent2.putExtra(SimDialogActivity.PREFERRED_SIM, 0);
                mContext.startActivity(intent2);
            } else {
                Log.d(TAG, "two sim card---slot has selected---");
            }
        }
    }
